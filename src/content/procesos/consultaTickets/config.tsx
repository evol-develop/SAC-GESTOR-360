...
export const TicketsMovimientos = () => {
  ...
  return (<>
    {ticketId ? (<>
    <Card className="w-[calc(130dvh-450px)] sm:w-[calc(277dvh-500px)]  h-[calc(165dvh-450px)] sm:h-[calc(150dvh-500px)]">
      <CardHeader className="flex flex-col items-center justify-between w-full sm:flex-row">
        <div className="flex flex-col w-full gap-1 sm:flex-row sm:w-auto">
          ...
          {!showArchivos && (
            <>
              <Button size="sm" className="text-xs " onClick={(e) => cambiarEstatus()} disabled={(etapaActual-1) === estatus.CERRADO}>
                <GrLinkNext />
                Siguiente etapa
              </Button>

              <Button size="sm" className="text-xs" onClick={(e) => OpenModalAsignarUsuario(e)} disabled={(etapaActual-1) === estatus.CERRADO}>
                <FaUserCheck />
                Asignar Usuario
              </Button>
